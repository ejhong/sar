...
ARGUMENT = [
    ("Radar does not go through rock",
     ...
     "anything the method knows about what is down there has to be read off how the surface moves.",
     "r14_budget"),
    ("So far the idea is reasonable",
     "A radar image gives a measurement at every pixel, millions of them, metres apart. That is a denser array "
     "than any seismic deployment on Earth. Shaking from distant oceans and cities passes through the ground "
    ...
    ("We ran the array test anyway, and nothing arrives",
     ...
     "is. We injected one into the same data and the method found it every time. The real data is flat.",
     "r11_array"),
    ("Where voids are certainly known, nothing appears either",
     "Giza's cemeteries hold hundreds of excavated burial shafts five to thirty metres deep, next to bare plateau "
     "with none. At the shafts' own depths, the two are indistinguishable.",
     "r13_known_voids"),
    ("And the depth numbers are manufactured",
     "The depth scale is set by a constant nobody measures, and the picture repeats every few metres. Two halves "
     "of the same pass put the same ground at different depths.",
     "r02_depth_axis"),
    ("The repeat is in the equation, not the data",
     "The depth stage matches each pixel against a sine and a cosine of depth. Those two curves come back to the "
     "same pair of shapes at regular spacing, so a peak at one depth has an identical twin below it, and another "
     "below that. The method's own documents say so. The elliptical part people call the spring model runs "
     "earlier and only chooses which pixels get scored, so it cannot break the tie.",
     "r15_modal_gate"),
]
...
<section class="db-title"><div class="wrap">
  ...
  such an array into an image of what lies below is standard seismology. The obstacle is time. That method needs
  minutes of continuous recording; one radar pass yields under two seconds, because a radar builds its image from
  a sweep of angles and waiting a second also means moving a degree, after which the ground no longer looks like
  itself. We ran the array test regardless, on 759 elements across 1.8 km: an injected wave was recovered every
  time, the real data was flat. Separately, at the known depths of hundreds of surveyed burial shafts nothing
  appears; a pyramid and open desert score the same in one acquisition; X-band reaches about half a metre into
  limestone; and the depth axis is periodic by construction with a freely chosen scale.</div>
</div></section>
